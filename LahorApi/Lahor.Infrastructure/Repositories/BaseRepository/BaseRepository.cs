...
namespace Lahor.Infrastructure.Repositories.BaseRepository
{
    public abstract class BaseRepository<TEntity, TPrimaryKey> : IBaseRepository<TEntity, TPrimaryKey> where TEntity : class
    {
        ...
        //public async Task SetTrackedValues<TDto>(IEnumerable<StatusTracker<TDto>> entitiesDto, bool isSoft = true) where TDto : class, IBaseEntity
        //{
        //    UpdateRange(entitiesDto.Where(x => x.Status == EntityStatus.Modified).Select(x => x.Object));
        //    await AddRangeAsync(entitiesDto.Where(x => x.Status == EntityStatus.Added).Select(x => x.Object));
        //    await RemoveRange(entitiesDto.Where(x => x.Status == EntityStatus.Deleted).Select(x => x.Object), false);
        //}
